...
const navItems: Array<{ name: string, id: number, goTo: string }> = [
  { name: "HOME", id: 1, goTo: "/Posts" }, { name: "FIND TALENT", id: 2, goTo: "/Posts" },
  { name: "BLOG", id: 3, goTo: "/Posts" }, { name: "SUPPORT", id: 4, goTo: "/Posts" },
];
...
const NavBtn = (p:any) => {
    ...
    return (
      <li >
        ...
        }}className="block py-2 lg:pl-3  text-gray-900 md:pl-1 cursor-pointer" >{p.name}</a>
      </li>
    );
  }
        ...
        <span className="sr-only">Search</span>
...

          {
            navItems.map((item : { name: string, id: number, goTo: string }) => {
              return (
                <NavBtn name={item.name} id={item.id} goTo={item.goTo} Nav={navigate} curUserData={curUserData} />
              );
            })
          }
          <button onClick={() => {
