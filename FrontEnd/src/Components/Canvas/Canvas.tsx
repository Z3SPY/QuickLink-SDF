...
class Point {
  ...
  update(
    ...
    mousePos: { x: number; y: number; radius: number }
  ) {
...
const settings = {
  dimensions: [window.screen.height * 0.7, window.screen.width * 0.4],
};
...
const Canvas: React.FC = () => {
  ...
  const [rotState, setRotState] = useState(
    "translate(0%, 0%) perspective(600px) rotateY(10deg) rotateX(10deg)"
  );
  ...
  function updateCanvasState(xRot: number, yRot: number) {
    ...
    setRotState(
      "translate(0%, 0%) perspective(1000px) rotateY(" +
        -rotationX +
        "deg) rotateX(" +
        rotationY +
        "deg)"
  ...
  useEffect(() => {
    ...
    context.drawImage(
      ...
      2.5,
      0,
      img.naturalWidth * ratio + 10,
      img.naturalHeight * ratio + 20
    ); //Resize Image
    ...
    window.addEventListener("mousemove", (event) => {
      if (catContext != null) {
        ...
        var lMouseX = Math.max(
          -200,
          Math.min(100, settings.dimensions[0] / 2 - globalMousePos.x)
        );
        var lMouseY = Math.max(
          -200,
          Math.min(100, settings.dimensions[1] / 2 - event.clientY)
        );
  ...
  return (
    <div id="Animated">
      <canvas
        ref={canvasRef}
        width={settings.dimensions[0] - 50}
        height={settings.dimensions[1]}
        style={{ transform: rotState }}
        className="positioning md:hidden sm:hidden lg:flex left-2/4 shadow-2xl"
        id="CatCanvas"
      ></canvas>
    </div>
  );
