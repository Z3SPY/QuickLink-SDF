...
const Canvas: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const cell = 8;
    const cols = Math.floor((settings.dimensions[0] - 100)/cell);
    const rows = Math.floor((settings.dimensions[1] - 100)/ cell);
    ...
    //Img
      ...
      context.drawImage(img, 0, 2, img.naturalWidth * ratio + 15, img.naturalHeight * ratio + 15); //Resize Image
...
window.addEventListener('mousemove', (event) => {
  ...
  return (
    <div>
      <canvas ref={canvasRef} width={settings.dimensions[0]} height={settings.dimensions[1]} class="positioning" id="CatCanvas"></canvas>
    </div>
  );
};
