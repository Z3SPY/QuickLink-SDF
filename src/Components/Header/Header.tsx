...
function listCheck() {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      ...
      list = document.querySelectorAll(".clickedButton");
...
const list : Array<any> = document.querySelectorAll(".clickedButton");
const nav = document.querySelector("navbar-sticky");

async function setUp() {
  console.log("waiting for list");
  const result = await listCheck();
  console.log("list is ready");

  list.forEach((item : any) => item.addEventListener("click",function(e : any){
    console.log(e.target);
    list.forEach((li : any) => li.classList.remove('active'));
...
const navItems : Array<navButton> = [ 
  ...
  new navButton("SHOP", 3), new navButton("SUPPORT",4)
]
...
const Header = () => {
  ...
  return (
      ...
      <nav className="bg-whitefixed w-11/12 z-20 top-0 left-0 border-b border-gray-200 " id="Header">
